...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (
      ...
      // New user created - send confirmation email
      const user = payload.record;
      if (user.email && !user.email_confirmed_at) {
        console.log("Sending confirmation email to:", user.email);
        await sendEmail(
          user.email,
          "Confirm your email - ticketsale.ca",
          getConfirmEmailTemplate(payload.record.confirmation_token)
        );
      }
    } else if (
      payload.type === "UPDATE" &&
      ...
      // User updated - check if email was just confirmed
      const user = payload.record;
      ...
      if (user.recovery_token && !oldUser?.recovery_token) {
        ...
        await sendEmail(
          user.email,
          "Reset your password - ticketsale.ca",
          getPasswordResetTemplate(user.recovery_token)
        );
      }
    }
...
function getConfirmEmailTemplate(token?: string): string {
  const confirmUrl = token 
    ? `${SITE_URL}/auth/confirm?token=${token}&type=signup`
    : '{{ .ConfirmationURL }}';
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #171717; border-radius: 24px; border: 1px solid #262626; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);">
                    ...
                    <tr>
                        <td style="padding: 0 40px 40px 40px; text-align: center;">
                            <a href="${confirmUrl}" style="display: inline-block; padding: 16px 32px; background-color: #ffffff; color: #000000; text-decoration: none; border-radius: 9999px; font-weight: 600; font-size: 15px; letter-spacing: -0.2px;">
                                Confirm Email
                            </a>
                        </td>
                    </tr>
...
function getPasswordResetTemplate(token: string): string {
  const resetUrl = `${SITE_URL}/login/reset-password?token=${token}&type=recovery`;
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Reset Your Password - ticketsale.ca</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Helvetica', 'Arial', sans-serif; background-color: #0a0a0a;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #0a0a0a; padding: 40px 20px;">
        <tr>
            <td align="center">
                <table width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #171717; border-radius: 24px; border: 1px solid #262626; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);">
                    ...
                    <tr>
                        <td style="padding: 0 40px 30px 40px; text-align: center;">
                            <h2 style="margin: 0 0 16px 0; font-size: 28px; font-weight: 700; color: #ffffff; letter-spacing: -0.5px;">Reset Your Password</h2>
                            <p style="margin: 0; font-size: 16px; line-height: 24px; color: #a3a3a3;">
                    ...
                    <tr>
                        <td style="padding: 0 40px 40px 40px; text-align: center;">
                            <a href="${resetUrl}" style="display: inline-block; padding: 16px 32px; background-color: #ffffff; color: #000000; text-decoration: none; border-radius: 9999px; font-weight: 600; font-size: 15px; letter-spacing: -0.2px;">
                                Reset Password
                            </a>
                        </td>
                    </tr>
